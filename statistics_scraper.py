...
# Testing the scrapping code. Prints out the values.
...
# Adding the data to the MongoDB database
def add_to_database():
    db = client['fantasy_music'] # create a new database
    overall = db['overall'] # create a new collection
    billboard_100_artists = db['billboard_hot_100_artists'] # create a new collection
    billboard_100_songs = db['billboard_hot_200_songs'] # create a new collection
    billboard_200_albums = db['billboard_hot_200_albums'] # create a new collection

    # delete all documents in the collection before adding new ones
    overall.delete_many({})
    billboard_100_artists.delete_many({})
    billboard_100_songs.delete_many({})
    billboard_200_albums.delete_many({})
    ...
    for i in range (0, len(billboard_hot_100_song)):
        billboard_100_songs.insert_one({
            'rank': i+1, 
            'song': billboard_hot_100_song[i],
            'artist': billboard_hot_100_song_artist[i],
            'artist_picture': billboard_hot_100_song_artist_picture[i]
        })

    for i in range (0, len(billboard_hot_100_artists)):
        billboard_100_artists.insert_one({
            'rank': i+1, 
            'artist': billboard_hot_100_artists[i],
            'artist_picture': billboard_hot_100_artists_picture[i]
        })

    for i in range (0, len(billboard_hot_200_album)):
        billboard_200_albums.insert_one({
            'rank': i+1, 
            'album': billboard_hot_200_album[i],
            'artist': billboard_hot_200_album_artist[i],
            'album_cover': billboard_hot_200_album_cover[i]
        })

    # identify each document with a unique ID. This is the primary key and is automatically generated by MongoDB.
